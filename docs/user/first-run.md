...
- a **hostname** — `mica-` followed by the first eight hex characters of the
...
- **no network configuration** — deliberately. The image ships a static DHCP
  ...
  any DHCP network without micad guessing interface names.
...
`mica-provisioning.toml`, at the root of a medium's filesystem, read once
