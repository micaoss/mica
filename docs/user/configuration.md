...
daemon (`micad`), stored as JSON documents under `/mica/config/` on the data
...
under `/mica/config/` on the data partition. What the device mints or observes
...
configuration reset returns everything under `/mica/config/` to the values the
...
the documented `/mica/config/` files requires a stopped device; live changes use
