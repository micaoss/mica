...
from its own CSPRNG, derives its `mica-xxxxxxxx` hostname from it, and mints
...
> status: shipped — evidence: `mica-system:overlay/usr/lib/mica/mica-seed-state`, `docs/design/provisioning.md`
...
- **A provisioning document at the factory.** The password is written into
  `mica-provisioning.toml` on the boot partition or a removable medium and
