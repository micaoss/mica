...
- Current startup helper set is B2 BusyBox + blkid + veritysetup + dmsetup.
  Retained exitrd is B3 static mica-shutdown; historical closure numbers above
...
mica-init is 2,403,504 bytes, SHA256
...
The startup observation path /sbin/mica-shutdown points to /exitrd/shutdown,
...
--producer boot --bins "mica-init mica-shutdown" --arch amd64.
...
The native output complement check confirms no mica-deploy signing tool was
emitted by this boot producer. Its source-file hashes were rechecked afterward.

| Output | Bytes | SHA256 |
|---|---:|---|
| Native mica-init | 2,403,504 | 64fda99a1f9f1b66fd32adac10071cf5f3987f34628b627381f40f1be04c331e |
| Native mica-shutdown | 2,047,144 | 5eaf5fa59306073fddce1868139628b126945c30f81c51faec0de59ae806adba |
...
The final x64 guest uses the new producer's mica-init worker entry point for loop
...
Fresh-target execution of the unmodified pkgs/mica-deploy/hack/check.sh passed
...
The original boot anchor, Mica OS signed-file policy, standard countdown/console,
...
"FAIL: pkgs/micad/apid/ui/verify-ui-policy.sh:82: an early-exiting grep on the right
...
The shared mica-deploy workspace/Cargo.lock/native-hook changes require a new
...
was launched. Unchanged J kernel/micad/podman/system inputs retain their original
identities if their relevant inputs are proven equal; no composition-only waiver.
B7's pending x64-only boot-tools route overlaps pkgs/mica-boot/Dockerfile and
...
- Applied exactly the reviewed patch SHA256
  ...
  pkgs/mica-boot/Dockerfile, pkgs/mica-boot/build-tools.sh and
...
- New mica-init: 2403504 bytes, SHA256
  57c865ed0b58740faaba642cc417a0b0a3a487f3b6718a1e2fcc7e1355bdea97.
  New retained mica-shutdown: 2047144 bytes, SHA256
...
- Retained signed metadata fixture:
  `consumer-signed-binding/evidence/signed-startup.micaupd`, 2019798 bytes,
...
- A final PMA-CR check found that the new archive validator allowed boot.json
  above mica-init actual 4096-byte reader budget. The added 4097-byte case first
...
- Actual deploy PREPARE ran once from clean 438c9551 and exited 0 at
  ...
  used 4 CPU / 10 GiB / equal swap / cpuset 4-7. The staged `mica-deploy` is
...
`mica/producer-join/startup-v1` with the four fixed J/438 receipt roles. They
...
### 2026-09-13: Phases 6-8 (mica-runkit, micad+apid, no curl/iproute2/iptables)

Producer-side implementation and measurements are recorded in the plan's
"2026-09-13: Phases 6-8 producer-side implementation" section. Uncommitted in
mica-deploy, mica-core, mica-system, mica-boards and mica-boot (parts of the
mica-boards and plan edits were swept into commits 4d26e22, 5ed255f and
d02e682 by a parallel session). mica-build adaptation waits for publication.

2026-09-13, status boundary: the producer changes are committed as
mica-deploy `5593860`, mica-core `bdb1a6a`, mica-system `d97d63b`,
mica-boards `9ee4dff` and mica-boot `5ac0371`. The user granted the push and
publication of exactly these commits and the mica record commit `76346eb`
("yes, push", 18:45:14); each owner pushes its own. mica-deploy's final
source is `b698b10`, a reviewed fix on top of `5593860` that bounds the
response head and chunk framing by the transfer limits; mica-build is
validating its exact binaries. The user has since asked for `mica-deploy` to
be imported into `mica-core` as a crate of one Cargo workspace. The user
approved that plan (`mica-core:docs/plan/20260913-1935-workspace-convergence.md`)
on 2026-09-13 at 19:45 UTC, asked at 19:44 for `apid` to be renamed
`mica-apid` and `micad` to be renamed `mica-core`, and authorized retiring the
old `mica-deploy` repository. `b698b10` is the intended runtime revision to
import. None of it is implemented yet: no path, package, unit or API name has
changed, so the `mica-deploy`, `micad` and `apid` names above stay valid. The
mica-deploy owner retires that repository only after the import keeps its
history and every artifact that depends on it is preserved; its source is
not deleted yet. A push is not a publication: an artifact
counts only once CI has published it and it reads back anonymously. The mica-build adaptation
(23 uncommitted paths) passed from its earlier worker to the mica-build
owner alone. Evidence so far is source, unit-test and package level (the
plan's "Phases 6-8 producer-side implementation" section); a new guest boot, the full product
composition and the physical CX acceptance (CAN, MAC, cold boot) are still
to run, so the task stays in progress.
