...
Move the software packages (micad, mica-deploy, podman) out of this tree into
...
- An x64 image composes from a pool whose micad, mica-deploy and podman archives
...
- Every mica archive records its source repository and commit in its control
...
Completed 2026-09-13: every phase of `20260911-2006-split-package-repositories` landed and was proven through the x64 release gate at `mica-build` `7afa4820`.
...
- Decided 2026-09-12: one repository per package, named for Mica OS
  ...
  names kept the former project prefix. Approved 2026-09-12 21:00.
...
- Phase 1 (2026-09-12 evening): provenance fields in every archive, `VERSION`
  ...
  `source-lineage.py` with `MICA_POOL_UNLOCKED`, the fixed producer join
  retired from the lineage script and the release gate, the package gate
  lock-aware, `os-pool` / `os-lock-bump`, `micad-build.txt` from the archive's
  field.
- Phase 1 proof (2026-09-12, worktree `mica-build` beside the workspace, branch
  `split/phase1`): the pool was built at `c87bfd1d`, `mica-podman` published
  ...
  and the per-package source, `micad-build.txt` came from the archive's
...
- 2026-09-12 23:20 (user): rename table confirmed; source moves to GitHub
  (`mica` on GitHub, main, one repository per package, local
  checkouts side by side in the workspace directory). Six package repositories created on
  GitHub, `origin` repointed, `MICA_SOURCE_URL` updated; the Debian archives
  stay on the Gitea registry. Retired worktrees `mica-arm64-package-source`,
  `mica-arm64-production` and `mica-discarded-20260912-1123` removed at the
  user's request; the Phase 1 worktree moved into the workspace as `mica-build`.
- 2026-09-12 23:40 (user): keep Gitea and update both forges in step; the
  ...
  continues in a fresh clone `mica` in the workspace. `git.ds.cc` was
...
- Phase 2 (2026-09-13, in the `mica` checkout): `mica-build-env` and
  ...
  beside the assembly in the workspace.
...
- Phase 0: throwaway archive uploaded, downloaded byte-identical, listed with
  its `Mica-Source-*` fields in the component index, deleted (201/200/204).
- Phase 1, offline: `tests/rootfs-runtime/source_lineage_test.py` (13, the
  ...
  pre-existing findings outside this change (`pkgs/mica-boot/init-keys.sh`,
  `pkgs/micad/apid/ui/verify-ui-policy.sh`).
- Phase 1, x64 proof on `split/phase1` (final commit `42b76d50`):
  `make os-pool` fetched `mica-podman` amd64 and arm64 through the lock and
  built the rest; `MICA_BOARD=x64 bash rootfs/build.sh` composed (198 MB)
...
- Phase 2 (2026-09-13, `3264236b`): `make docs-verify` green;
  ...
  smoke 12/12, the archives now saying `Mica-Source-Repo: mica`.
...
- 2026-09-13 (user, three decisions in one night): archives are published
  ...
  `mica-podman` round trip through a real release (`build-199cf440684e`:
  published, pinned, deleted locally, fetched by `make os-pool`, x64
  composed with smoke 12/12).
- 2026-09-13 04:10 (user): keep `mica-build` for the assembly; `mica` is a
  new project management and documentation repository. Done on both forges
  and locally; `docs/` and `tools/docs/` moved here with history, citations
  into code rewritten as `mica-build:<path>`, the assembly keeps executed
  copies of the two documents its tests read; `make docs-verify` and
  `docs-verify-test` green here.
