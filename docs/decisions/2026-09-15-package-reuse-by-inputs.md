...
- **status**: accepted (user, 2026-09-15); not implemented; applies to `mica-boards`; whether `mica-core`, `mica-podman` and `mica-system-base` adopt it is not decided
...
- only a package whose inputs changed is rebuilt, with the new commit
  version;
- the inputs hash is recorded with the published package; `mica-boards`
  proposes where (for example a pool layer annotation `mica.inputs`);
