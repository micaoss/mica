# Development builds leave the release; the released variants are basic (the default) and full
...
- **What a board releases is two variants.**
  ...
  - **full** carries every feature the board supports.
...
1. **The names.** `<board>-basic` and `<board>-full` are the obvious products. Which one takes over
   what `<board>-prod` is today, and which is new?
...
between feature and footprint. The smaller one is the default: a device that needs the engine
takes full on purpose.
