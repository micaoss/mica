...
> mica-derived system on SSD202D would have to give up, what it can keep, and
...
|---|---|---|---|
| 1.1 | Core | Dual Cortex-A7, **ARMv7 32-bit** | Mica OS has no 32-bit target; `MICA_ARCH` is `amd64`/`arm64` only |
...
| One deployment (`.micaupd`) | 154.5 MiB | 9.7× |

The root is 260 MiB on disk before squashfs. Its three largest contributors are
`mica-podman` at 83.5 MiB — a third of the image — the five Mica OS Rust binaries at
...
and the MQTT pair, while keeping Debian, systemd, micad and apid:

| Composition | dpkg | on disk | squashfs |
|---|---|---|---|
| No podman / netfilter / MQTT | 245.1 MiB | 174.4 | **51.4 MiB** |
| + Rust `strip` and LTO (~50% off the 27.0 MiB of micad/apid/deploy) | 231.6 MiB | 164.8 | **48.6 MiB** |
| + armv7 against arm64 (~12% smaller) | | | **~42.7 MiB** |

**A Debian armv7 read-only root carrying systemd, micad and apid is roughly
...
| micad / apid / deploy | fused into one multi-call binary | three binaries as today |
...
The 128 MiB path adds a board to mica. The difference is not eight times the
flash; it is an order of magnitude of engineering and the entire long-term
cost of maintaining two trees.

At 128 MiB the work reduces to four items: an armv7 target (`MICA_ARCH=armhf`
...
| The per-slot signed `mica_rootfs_desc` and U-Boot's extra `rsa_verify()` | §6.3 |
...
- **Premise 1.2 becomes the decisive one.** systemd, journald, udev, dbus,
  micad and apid together are a rough 60–80 MiB resident. With a 32 MiB MMA
...
- **Streaming installation.** Mica OS's native acquisition stages a whole
  deployment in DATA under `/mica/updates/{staging,downloads,verified}`. Here
...
of margin, and it depends on one decision: **micad, apid and the deploy agent
...
Two facts about the current tree bear on it: `pkgs/micad/Cargo.toml` declares no
`[profile.release]`, and the deb producer runs a plain `cargo build --release`
with no strip step, so the shipped binaries carry their symbol tables — `micad`
...
userspace ELF closure that `pkgs/mica-boot/initramfs.sh` assembles, and
`mica-init` itself. On a 128 MB part that is not an optimization, it is what
...
`boards/common/mica-records.h`. Omitting this step does not fail loudly — it
...
|---|---|
| `boards/common/mica-records.h` | Near-verbatim. Pure ANSI C over `stdio.h`/`string.h`, no U-Boot headers. At 16 MiB it drops `kernel[65]`: with no independent kernel component there is no second identity to bind. At 128 MiB the field stays. |
| `mica-file-boot.c` (cx3576) | Shape, 205 lines. Block reads become MTD or `ubiblock` reads; the FIT path lookup becomes a fixed offset or a UBI volume. Redundant-copy selection, CRC, trial decrement-before-load and refuse-on-persistence-failure all carry over. |
...
Dropped at either capacity: podman and Quadlet, `mica-mqttd` and the broker, and
...
`pkgs/micad/micad/src/reconciler/` are written against systemd units; the adapter
