...
| Mica OS's own five Rust binaries (`apid`, `micad`, `mica-mqttd`, `mica-mqtt-broker`, `mica-deploy`) | 27.6 MB | **11.5 MB** | 16% |
...
`mica-system`'s own control file already states why they are there: operator
...
`alsa-utils`, `mica-podman` — are already declinable, per board or per build.
...
Sixteen shell scripts under `/usr/lib/mica` carry the seed, reconcile, health,
...
`mica-health` probes apid exactly once, with
...
`mica-data-layout` applies project quotas by execing `setquota`. The `quota`
package plus `libext2fs2t64` is 2.09 MB unpacked; the operation is
`quotactl(2)`.

### 3.3 `ssh-keygen` — `openssh-client` for host-key generation

`mica-seed-state` generates the sshd host keys on first boot with `ssh-keygen`.
...
**The release profile is not set.** `pkgs/micad/Cargo.toml` declares no
...
**Five binaries are five copies of one dependency graph.** `micad`, `apid`,
`mica-mqttd`, `mica-mqtt-broker` and `mica-deploy` are members of one cargo
...
is Rust, chiefly the axum and utoipa surfaces. `mica-mqttd` and
`mica-mqtt-broker` are 4.6 and 4.3 MB and both exist only because micad exists.
...
- **`bluez`, `wpa_supplicant`, `hostapd`.** Protocol and certification
  surface. `wifi.md` already places them correctly: micad drives their unit
...
- **systemd, udev, `libsystemd-shared`.** The boot order, the mount topology,
  verity activation, `systemd-repart` growth and the unit lifecycles micad
...
which `mica-ca-trust` and `mica-busybox` are unconditional.
...
## 7. Network configuration: could micad own it directly?

### 7.1 What the coupling is today

micad does not configure the network; it renders files that something else
...
micad serves to apid is assembled from networkd's `Describe`, sysfs, the
...
- **A group-readable secret leaves the image.** The WireGuard private key is
  ...
  micad programs WireGuard through the kernel's generic-netlink family, the key
  never leaves micad's address space and the file, the group and the assertion
  all go.
...
| `network-online.target` and `systemd-networkd-wait-online` | micad becomes a boot-ordering barrier for anything that waits on the network | Medium — an architectural consequence, not a coding one |
| The `Describe` surface | micad's own rtnetlink dump replaces it | Low, and arguably an improvement: the observed-state module already renders rtnetlink enumerations by name |
...
and parsing stdout. micad calls `org.freedesktop.network1.Manager.Describe` over
...
**Their UI evolution independently reproduces micad's shape.** Venus issue 1215
...
micad owns static, VLAN, bridge and WireGuard ones. The cost is that two
...
| Mica OS today | `micad`, shipped | systemd-networkd | no |
...
the Bottlerocket comparison, "micad should own its own network programming"
...
|---|---|---:|---|
| 1 | `[profile.release]` in the micad workspace, then the multicall merge | 3–5 MB *(expected, unmeasured)* | one repository |
| 2 | Trim `gconv` at the pack stage, with an assertion on what still resolves | 3.35 MB | one pack script |
| 3 | `mica-health`'s curl probe in Rust; drop the 15-package closure and the probe's SKIP path | ~1.7 MB | micad/apid, `mica-system` depends, health gate |
| 4 | Host-key generation in Rust; drop `openssh-client` and `openssh-sftp-server` | ~1.5 MB | seed script, `mica-system` depends |
| 5 | Network programming via rtnetlink (§7.7); drop the `ip link del` shell-out and, with §3, `iproute2` | ~2.6 MB | micad, and it is the largest piece of work here |
| 6 | Delete the iptables family, the dpkg residue, and decide on `e2scrub` | ~0.6 MB | `mica-system` depends, purge script |
