...
- Access channels are **first-class services under the model layer**
  ...
  executor is micad over systemd.
...
Access policy is a subtree of the micad settings tree (`docs/design/micad.md` §1),
persisted as one document per reconciler under
`/mica/config/` on DATA plus the remainder on DATA/state at
`/var/lib/mica/settings.toml`, each document carrying its own schema version
(`docs/design/micad.md` §2.1a says which key lives where). Addressed as one tree
regardless:
...
about that exact path. The only writers of `/etc/shadow` today are micad's bus
method (§4.1) and `mica-shadow-reconcile` at boot.
...
a normal filesystem. micad reconciles the whole tree at every start, so the unit
...
the SSH server only. sftp (and with it scp from OpenSSH 9 and later clients,
which use the SFTP protocol) comes from `mica-sftp-server`, `mica-core`'s
...
- `dropbear.service` is disabled on both image profiles by
  `50-mica-dropbear.preset` (`disable dropbear.service`), and the package's
  postinst refuses a `multi-user.target.wants` link. micad enables and starts
  the unit at runtime scope only after it has written a validated
  `/run/mica/dropbear.env`.
- `EnvironmentFile=/run/mica/dropbear.env` is required, so a missing file
  fails the start, and `mica-dropbear-prestart` refuses arguments that name no
  `-p` listener: Dropbear without one listens on port 22 on every address.
- The unit requires and follows `mica-shadow-reconcile.service`, because
  Dropbear reads `/etc/shadow` for password logins.
- The host key is `/mnt/data/state/ssh/dropbear_ed25519_host_key`, made at the
  first start and never at build time. An existing OpenSSH ed25519 host key is
  converted, keeping the device's fingerprint. A symbolic link, a foreign owner
  or permissions that others can write or read are refused rather than
  replaced by a new identity.
- There is no `etc-ssh.mount`.
- `KillMode=process`: Dropbear has no reload, so micad applies new arguments
  with a restart, and established sessions survive a restart or stop, as they
  did before.

Evidence (local): the package gate passed 130/130 and the systemd test
(`mica-system:tests/dropbear-systemd-test.sh`) 29/29, including a session
surviving a restart under `KillMode=process` and dying under `control-group`,
a start refused without the environment file, and a key login.
...
- The `access.ssh` reconciler (still named `sshd`, and still the live-state
  ...
  `-p <addr>:<port>` per listen address, IPv6 in brackets, or `-p <port>` when
  the list is empty, which keeps §3.2's "empty means every address"; at most
  ten addresses, Dropbear's own limit, and a longer list is refused rather
  than silently truncated. `-s` is added unless password authentication is
  effective, which stays `passwordAuthentication` AND an active transient root
  password; `-w` is added when `permitRootLogin` is false; `-g` is never used,
  so root login stays a yes/no setting.
- Keys are managed as home files, the configuration this design chooses:
  `~/.ssh/authorized_keys` of `root` and `mica` (homes as `/etc/passwd` names
  them), owned by the account, 0600 in a 0700 `~/.ssh`, written through
  directory descriptors that follow no symbolic link. A home that is group- or
  world-writable or owned by someone else fails the apply before anything is
  written; a missing account or home is skipped. `micad.service` no longer
  sets `ProtectHome`.
- When a rendered file changes under a running server, micad restarts
  `dropbear.service`; `KillMode=process` above keeps established sessions.
  A failed unit is reset before micad starts it, and the files are always
  written before any unit call.
- The published state carries `environmentFile` (formerly `dropIn`),
  `authorizedKeysPaths` and the unit `dropbear.service`.
- `mica-sftp-server` is an SFTP version 3 server on standard input and output
  (russh-sftp), running as the logged-in user. It advertises no extensions,
  refuses paths that are not UTF-8, and does not serve the legacy SCP protocol
  (`scp -O`).

Evidence (local, source and package level): `make check` with 1104 tests
and the package gate 91/91 on `ae513fb`; 1109 tests and the package gate
91/91 on the rebased commits, including a source-level test that the files
precede every unit call; and an interop run of the pinned `dropbear-bin`
2025.89 with OpenSSH 10.0 clients over the layout micad renders (14 cases:
sftp transfers and file operations, scp and `scp -r` in SFTP mode, `scp -O`
...
false on every image (`mica-core:crates/micad/src/provisioning.rs`; there is
no image profile to read, §5.3), and neither image ships `ssh.service`
enabled — `mica-system` ships
`/usr/lib/systemd/system-preset/50-mica-ssh.preset` holding `disable
...
(`mica-system:system/Dockerfile`). Getting in requires
...
`mica` is a **persistent working directory and a non-root default shell, not a
...
calls a micad bus method, which:
...
`mica-shadow-reconcile` — ordered before `micad.service` and `ssh.service` —
...
**Why a marker rather than "lock root on every boot".** Any root hash micad did
...
`/var/lib/mica/secrets/device-password` are still minted at first boot and still
...
Both later phases authenticate **directly against micad** — a derived PIN checked
...
to claim it, and until 2026-09-04 nothing made that pair one step: micad
...
seam anywhere in it: against a real micad on a real bus, two concurrent
...
not in micad because apid is the only claimant for as long as the device can be
asked — the other writer that can create a first `webAdmin` is the document
importer, which runs to completion before micad requests its bus name, so no
request can be in flight beside it. A third claimant arriving inside micad would
...
record and the API token the route mints are three keys of one subtree, so micad
...
### 5.3 Dev and prod images — no profile package and the `mica.profile` carrier **[decided]**, the carrier and a sealed image **[not implemented]**

Decision 2026-08-17: prod ships SSH. Development and production images exist,
and that pair is complete rather than half of a third.

**There is no image profile package and no profile file** (user decision
...
**Dev and prod differ by the kernel command line parameter
`mica.profile=dev|prod` — [decided] (user, 2026-09-14), [not implemented].**

- `mica-build` writes it for every image, prod included, from `PROFILE` in
  `mica-build:products/<name>/product.env`, and only into signed boot
  configuration: on UEFI boards the UKI's signed `.cmdline` (and signed UKI
  profiles or add-ons), on FIT boards the kernel's forced built-in
  `CONFIG_CMDLINE` inside the signed FIT. Every command line variant of an
  image, recovery and boot-counting entries included, carries exactly one
  identical token; each A/B slot carries its own signed value. The writer is
  implemented on an unmerged `mica-build` branch.
- `micad` reads it once at start. Exactly one token `mica.profile=dev` means
  dev; anything else (absent, `prod`, empty, another spelling, duplicated)
...
- The command line is enforced on uefi-x64 and uefi-arm64 only with UEFI Secure
  ...
  its own line) but not against serial-console access: both U-Boots keep an
  interactive console with memory and device-tree commands, one U-Boot serves
  both profiles, and the boot ROM does not verify U-Boot. That is an
  accepted, known limit (user, 2026-09-14): no hardened prod U-Boot for now. Where enforcement is not established, dev
  may only relax diagnostics or convenience.
- The allowed effects are decided (user, 2026-09-14): reporting the profile
  in `system_info`, diagnostic verbosity and log retention, and developer
  convenience that grants no access. Access defaults, credentials, the update
  channel, trust anchors and signature checks, network exposure and
  state-changing `apid` endpoints never depend on it.
- The profile is independent of `trust.grade`: a prod image may ship with
  development-grade signing keys.

No dev/prod difference exists today; the allowed effects above are the only
ones `mica-core` may implement. The record is
`docs/decisions/2026-09-14-no-image-profile-packages.md`.

**A product-level defaults tier is planned between the code defaults and
DATA/state** (`mica-build:products/<name>/defaults.toml`, plan
`20260913-0416-board-product-build-architecture` section C3): non-secret
settings defaults such as `access.ssh.enabled`, validated so that a prod
product may enable SSH only with password authentication off, and never a
password or PIN -- those travel only in the product's `provisioning.toml`,
which is the boot-time document of `provisioning.md` §4.1 placed on the
image's boot medium and marked `factory-seeded`. The `micad` reader is
`20260913-0440-micad-product-defaults`; until it lands the file is
validated and not composed.

The `ROOT_PASSWORD` build arg is **v1-only** and is not selected by the profile
on Mica OS: `src/rootfs/build.ts` and `stages/compose/` carry no such plumbing,
...
transient password set at runtime through micad, plus the serial console, whose
...
apid keeps its counters and TLS material in `/var/lib/mica/apid`, backed by
...
deliberately — not an overlay. `mica-system` ships **eight enabled core binds**;
...
|---|---|---|---|
| `mica.mount` | `/mnt/data/mica` | `/mica` | **DATA** |
...
| `var-lib-mica.mount` | `/mnt/data/state/mica` | `/var/lib/mica` | DATA/state |
| `home.mount` | `/mica/home` | `/home` | **DATA** |
| `root.mount` | `/mica/root` | `/root` | **DATA** |
