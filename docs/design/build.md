...
can emit ARM64 binaries without executing them. Whether a local arm64 build
matches the released one is a question per build, and the split is **on-target
versus cross-compiled**, not one language against another: a stage whose
container runs on the target platform differs from CI only by emulation, which
reproduces (measured 2026-09-16: `mica-system-base`'s eight archives,
`mica-podman`'s whole offline build), while a stage that runs on the host with
a cross toolchain against a natively built CI is two different builds, which
differ (`mica-core`, six Rust packages of six). C cross-built would behave the
same way, and Rust built on target would very likely reproduce; the language
is not the test. A local arm64 rebuild that differs from a release is
therefore not evidence of a changed input until the control has been run, and
no version is bumped for it; the architecture's answer comes from CI. Root package scripts and binary
