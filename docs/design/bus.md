...
- `com.mica.micad1` is a local system-management API. APID is its network-facing
  client; the boot-health gate is its other approved local caller.
- `com.mica.<class>[.<suffix>]` is the uniform service-name grammar. `micad` is
  therefore an application name too; the name alone grants no capability.
- `mica-mqttd` mirrors only exact service names enrolled by their package. It
  never calls or subscribes to `com.mica.micad`, even if a malformed package
  tries to enroll that name.

The old `com.mica.Item1` projection on `com.mica.micad` has been removed. It was
...
System functions stay on `com.mica.micad1` and APID. In particular, MQTT does
...
The bridge makes no micad call. micad renders the already-provisioned topic
identity to `/run/mica/mqttd-device.env` before starting the bridge, and systemd
passes it through `--device-id`. The file is a one-purpose runtime input, not a
general settings export.

micad still has a D-Bus interface because micad and APID are separate local
...
This is a fail-closed boundary. `mica-mqttd` constructs an application identity
only when `/usr/lib/mica/mqtt-applications.d` contains a regular file whose file
name is that exact, valid D-Bus service name. `com.mica.micad` is rejected
structurally after loading as a second defence. A sibling service, a wildcard,
or a valid but unenrolled `com.mica.*` name cannot enter bridge state or become a
read/write target.
...
Applications expose `com.mica.Item1`:

```xml
<interface name="com.mica.Item1">
...
`NameOwnerChanged` match for the `com.mica` namespace, then performs `ListNames`.
...
1. an empty enrollment file named for the exact service, for example
   `/usr/lib/mica/mqtt-applications.d/com.mica.sensor.example`; and
2. an exact-name D-Bus policy that lets the application own that name and lets
   `mica-mqttd` access only its `com.mica.Item1` surface.
...
It also grants root `GetItems`: micad's registry (section 7) probes the
service with that call, and the stock system bus denies method calls by
default with no exemption for root. For example:

```xml
<policy user="mica-sensor">
  <allow own="com.mica.sensor.example"/>
</policy>
<policy user="mica-mqttd">
  <allow send_destination="com.mica.sensor.example"
         send_interface="com.mica.Item1"
         send_member="GetItems"/>
  <allow receive_sender="com.mica.sensor.example"
         receive_interface="com.mica.Item1"
...
service. `com.mica.micad` is forbidden on both sides.
...
The bridge uses one mica-native grammar:
...
`mica-mqtt-broker` is the local `rumqttd`-based broker. `micad`'s MQTT reconciler
...
`mica-mqttd` is disabled in the immutable image and runs as the static,
unprivileged `mica-mqttd` account. Broker connection settings come from the
optional DATA/state-backed `/var/lib/mica/mqttd.env`; the default bridge mode is
`read-only`.

For authenticated MQTT, enroll a broker account in the existing
`/var/lib/mica/mqtt-broker-users.toml` `[users]` table, then provision the same
username and password in `/var/lib/mica/mqttd-credentials.json`:

```json
{"username":"bridge","password":"<the enrolled password>"}
```

The broker account file must remain readable by `mica-mqtt-broker`. Own the
bridge JSON file by `mica-mqttd:mica-mqttd` with mode `0600`, then restart the
...
## 7. micad's service registry

The management daemon separately observes every other `com.mica.*` service for
...
A conforming service exposes `com.mica.Item1.GetItems` at `/` and includes these
...
usable device instance is reported under `conformance`; it does not make micad
...
The registry describes every `com.mica.*` service. MQTT admits only exact
package-enrolled application services and independently fails closed on
collisions.

## 8. Sparkplug B

Sparkplug B is not implemented by `mica-mqttd`. Its birth/death certificates,
...
the same exact-enrollment boundary and structural micad exclusion; it must not
