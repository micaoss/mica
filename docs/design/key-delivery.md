...
| Metadata | Deployment/catalog/firmware publisher | Embedded in authenticated kernel policy | `mica-init` and `mica-deploy` authenticate strict Ed25519 envelopes |
...
composer receives public factory defaults through `MICA_META_DIR`; it receives
...
mismatched inputs. `MICA_SIGNING_OUTPUT` overrides the directory. Key generation
