...
- DATA and its state/meta namespaces are plain ext4. Settings, the apid admin password
  ...
  (`/var/lib/mica/secrets/`, `docs/design/provisioning.md` §3.4) are readable
...
media read-only. cx3576 has raw FIRMWARE and uses removable media. micad validates
...
is observable (micad's diagnostic bus registry; the verity root itself stays
