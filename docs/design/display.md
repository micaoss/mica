...
`DisplayConfig` → controller → `DisplayStatus` resource → mica-gui container
...
- kernel: DRM/KMS `=y` for the SoC display pipe + HDMI encoder; GPU driver
  ...
  already asserted.
- GPU userland (mesa) into the mica-gui container image; kernel DRM and GPU
...
- Output and default rotation, which are still a comment in the board
  ...
  (`boards/cx3576/board.env`). the `display` feature says whether there is a
...
| Phase | Scope |
|---|---|
| 1 | mica-gui container (cog/WPE), DisplayConfig/controller, apid local wizard route, splash-to-kiosk handoff on cx3576 |
