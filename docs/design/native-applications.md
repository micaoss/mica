...
Two ordering facts are mica-specific and both are load-bearing:
...
| `/mica/apps` | Managed application data | Unlimited DATA usage and application reset scope |
...
- **The required set is a build-time decision.** `/etc/mica/health.conf` is
...
- **A failed unit is still visible.** The gate reports the count and the names
  to micad at live-state `health.units`, one journal line per unit, and the
  entry ships in a diagnostic snapshot under `failures.health`.

## 7. Named hardware

Device access is systemd's, with nothing mica-specific over it:
...
bound takes down micad, apid and the update path with it, and the first
