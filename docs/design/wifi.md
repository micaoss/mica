# Design: Wi-Fi station and access point

> Wi-Fi station and access point on the systemd/micad base. Companion to
> micad.md, access.md and provisioning.md.

## 1. What ships

There is no separate connectivity daemon. Wi-Fi is carried by:

- **two micad reconcilers**, `wifi_client.rs` and `wifi_ap.rs`, driven from the
  micad settings subtrees `wifi.client` and `wifi.ap` (schema v3);
- **systemd**, which owns the unit lifecycles — micad never supervises a daemon
...
- **micad's live-state tree**, served over D-Bus to apid, as the only status
  surface.
...
**CAN and Bluetooth are not micad's.** They stay with the board hwinit units
(`hwinit-can`, `hwinit-bt`), which are already read-only-root safe and already
work; folding them into micad would rewrite something that is not broken for no
benefit. The boundary is therefore: `wifi.client` / `wifi.ap` are micad's, CAN
and BT are the board layer's.

## 2. Settings model

Both subtrees live in the micad settings tree (`docs/design/micad.md` §1) and are
persisted in `/mica/config/wifi.json` on DATA, at that
document's own schema version (`docs/design/micad.md` §2.1a). Both Wi-Fi
...
- **No secret has a non-`None` default.** `wifi.ap.ssid`, `wifi.ap.psk` and
  ...
  build.
- **`wifi.client.networks` is a whole-array write.** micad's dot-path API
...
`wpa_cli save_config` cannot rewrite micad's render.
...
`provisioning.deviceId` as `mica-<first 8 hex>` — the same shape the derived
hostname uses, so a device advertising `mica-1a2b3c4d` is recognisably the device
called `mica-1a2b3c4d`. The key is `wifi.ap.psk` when set, else
`/var/lib/mica/secrets/ap-psk`, the per-device secret minted at first boot. There
...
AP's key comes from DATA/state, so on an unprovisioned device there is nothing to
...
`mica-core:micad/src/reconciler/network.rs` **deletes every `*-mica-*.network` file it
did not itself render.** A WiFi unit named `50-mica-wlan0.network` is therefore
swept away on the network reconciler's next pass: the component is present,
every unit test is green, and WiFi has no address on device. A future reconciler author must not rediscover it.
...
| Renderer | Prefix | Why |
|---|---|---|
| `network.rs` (ethernet, operator-configured) | `50-mica-` | owns the sweep |
| `wifi_client.rs` | `90-wifi-client-` | no `-mica-`, so the sweep misses it |
...
- the prefix does **not** contain `-mica-`, so `network.rs` does not match it;
- `90-` sorts after `50-mica-…` and after `80-dhcp.network`, and networkd applies
...
running DHCP on a link micad no longer manages.
...
- no secret enters the image. Every key in a rendered file comes from the
  settings tree on DATA/state or from `/var/lib/mica/secrets/ap-psk`, both born at
  runtime.
...
the instances micad starts:

| Unit | What the package does | Why it is a problem |
|---|---|---|
| `wpa_supplicant.service` | shipped **enabled**, with **no condition gating it** | it *does* start. Worse, it carries `RuntimeDirectory=wpa_supplicant`, so systemd **deletes `/run/wpa_supplicant` when it stops** — taking the control socket of the templated instance micad started with it |
| `hostapd.service` | shipped **enabled** | inert *today* only because its `ConditionFileNotEmpty=/etc/hostapd/hostapd.conf` is unsatisfied. That is one operator `cp` away from a second hostapd on the same radio while micad's instance reports healthy |
...
The templates micad *does* drive (`wpa_supplicant@.service`, `hostapd@.service`)
must **not** be statically enabled in the image: micad owns those lifecycles and
...
| Config dirs | `/etc/wpa_supplicant` and `/etc/hostapd`, DATA/state-backed binds via `etc-wpa_supplicant.mount` / `etc-hostapd.mount`, mode 0700 |
...
The verifiers read every path, prefix and unit name **out of the micad source
that owns it** rather than restating it. A constant restated in two places can
drift, and this drift is invisible from the code side because micad's tests all
pass against a mock. Each extraction is checked for emptiness, so a rename in
micad breaks the verifier loudly instead of turning an assertion into a
comparison against `""`.
...
- **Passphrase length is not validated** on the station side. wpa_supplicant
  ...
  taking every network down. Validation belongs in `micad-settings` with the rest
...
- **A stale `<old-interface>.conf` and its unit are not swept on an interface
  ...
  stopping units for interfaces micad was never told about is a broader claim of
  ...
  instance, and micad only ever starts the configured one — so a reboot clears it.
- **`write_atomically` is duplicated three times** — `sshd.rs`, `wifi_client.rs`,
  `wifi_ap.rs`. Lifting it into a shared helper is worth doing.
- **Regulatory correctness cannot be verified in this repository.**
  ...
  Two of those three are worth stating exactly, because
  the first is easy to mistake for the whole answer. **The database is now in
  the image**: `mica-wifi` and `mica-wifi-ap` depend on `wireless-regdb`, and the
  ...
  clears it is nl80211's `RELOAD_REGDB`, so `mica-wifi` also ships
  `mica-regdb-reload.service`, a oneshot running `iw reg reload` before
  ...
  **The driver is the third element and it is still open.** On the AIC8800D80
  ...
  question. `docs/boards/cx3576-bench.md`'s `regdb-loaded` probe records both.

## 10. What is NOT claimed

Neither reconciler has been tested against a radio. Every test in both reconcilers runs against
