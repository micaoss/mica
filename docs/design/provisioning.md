...
and is thereafter a fully working, configurable appliance. `micad`'s
...
- **hostname `mica-<first 8 hex of deviceId>`**;
...
- **The module references no networking API at all** — its entire import list is
  `std::fs`, `std::path::Path`, `anyhow`, `micad_settings` and `crate::identity`.
...
- **`network` is seeded empty on purpose.** The image already ships a static
  ...
  exist. It also keeps micad's network reconciler from claiming ownership of an
...
still equals the built-in default `"mica"`.

A provisioning failure **aborts micad startup**, deliberately. An unwritable
...
operator cannot log in to. Under `MICAD_DRY_RUN=1` provisioning is skipped
...
are separate and predate this work: `mica-seed-state` generates them into
...
|---|---|---|
| **device password** | hash in `access.device.passwordHash`; plaintext at `/var/lib/mica/secrets/device-password` (0600 in a 0700 dir) | **nothing, since 2026-08-19** — not SSH, not the local console, not the apid admin UI. See §3.6 |
| **AP PSK** | plaintext at `/var/lib/mica/secrets/ap-psk` (same modes) | WPA2 clients joining the provisioning access point |
...
|---|---|---|
| `access.device.passwordHash` (settings tree, on DATA/state) | **Argon2id** PHC string | micad and apid, in Rust, against themselves |
...
where micad and apid verify against themselves, and libcrypt is not involved
there at all.
...
behind the daemon's back. That is what `docs/design/micad.md` §1 describes. §4.1
...
### 4.0 The pour: writing `/mica/config/` directly, and why it is not a sixth channel
...
motivating case for the whole namespace. `docs/design/micad.md` §2.1a is the
rule list; what belongs here is where it sits relative to the five channels
above.

**It is not a channel, because it is not a document micad reads and applies.**
...
writes exactly the documents micad would have written, and micad reads them on
...
- **Offline only.** A pour is written onto a device that is **not running**.
  ...
  a *running* appliance.
- **Validated on the next boot, exactly as micad validates its own output.**
  ...
  chose.
- **A poured secret is not read back.** `/mica/config/wifi.json` carries the
...
documents written into `/mica/config/` before micad exists are validated on the
...
`docs/design/micad.md` §2.1a is the rule and the per-document cover behind it.
...
- **the missing `/mica` medium** (F6f) — a device that cannot reach its
...
on a boot partition with any text editor. The file is `mica-provisioning.toml`
...
predicate in `micad-settings` to reuse — writing one here would be the second
...
**Atomicity** covers the complete settings import across DATA/mica/config and DATA/state.
...
`/var/lib/mica/settings.transaction.json`, containing the previous bytes or
...
- **The pour** is a write to the DATA partition of a device that is **not
  ...
  write `/var/lib/mica/settings.toml` and set `access.webAdmin`'s password hash
...
`mica-provisioning-import.service` runs `Before=micad.service` and after
`local-fs.target`. It mounts a candidate **read-only** (`ro,nosuid,nodev,noexec`)
under `/run/mica/provisioning/<source>` and keeps the mount only when
`mica-provisioning.toml` under it is a **regular file** — a directory, a symbolic
link planted on the medium, or a device node leaves nothing mounted. micad
...
reports what micad did with a document it was given; a medium that would not
mount, or a filesystem type the kernel does not have, never becomes a document,
so there is nothing for micad to record. The unit says which source it staged,
or that it staged none, and `journalctl -u mica-provisioning-import` is where an
...
`micad_settings` and `crate::identity` — no socket, no resolver, no DHCP lease,
...
#### 4.1.7a The document as a factory seed

A product may carry the document itself
(`mica-build:products/<name>/provisioning.toml`): the assembly places it on
the image's boot medium as `mica-provisioning.toml`, exactly where channel 1
reads it, so a device flashed with that image provisions itself on first
boot from the product's seed. Nothing about the transport changes -- the
same file, the same fixed name, the same already-claimed rule -- and the
composition record and the lineage mark the build `factory-seeded`, which
the release gate refuses on the production channel. The seed is the one
place a product may state a secret; `defaults.toml` beside it may not.
