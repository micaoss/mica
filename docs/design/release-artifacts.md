...
bytes. It uses only `mica/release/v1` and `MICAUPD1`. The update archive contains a
...
| Filename | Content |
| --- | --- |
| `mica-BOARD-YYYYMMDD-HHmmss.img` | Complete current three-partition factory image, named by UTC build time |
| `update.micaupd` | `MICAUPD1` deployment envelope and authenticated objects |
| `firmware.json` | Independently signed `mica/firmware/v1` envelope |
| `firmware.bin` | Firmware bytes named by that envelope; the original destination remains in its signed target |
| `package-manifest.tsv` | Root image's `/usr/share/mica/manifest.tsv` |
| `baked-meta.json` | Root image's `/usr/share/mica/meta/updates/manifest.json` defaults |
...
| `builder-images.json` | `IMAGE_*` and `LOCAL_*` values from `mica-build-env:images.env` |
...
bash build/run.sh --release assemble \
  ...
  --package-manifest /absolute/extracted/usr/share/mica/manifest.tsv \
  --baked-meta /absolute/extracted/usr/share/mica/meta \
