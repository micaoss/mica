...
> to api.md, micad.md, storage.md and time.md. The Rust/API core ships; the
> built-in UI pages and the operational procedures are a follow-on
> (section 10).

## 1. What ships, and what deliberately does not

Four read-only observations on `com.mica.micad1`, each behind a trait with an
...
| Observation | micad module | Bus member | Route |
|---|---|---|---|
...
`/usr/share/mica/meta`. It is provenance, not an active firmware attestation.
...
product rather than the distribution. The version comes from the **same
expression the signed components take theirs from**, so the console banner,
...
| `boot` | `deployment`, `uptime` (section 2), `reset` (section 4), `update` (micad's recorded live-state `update` entry: `boot` identities and verification, `state` with `current`, `fallback`, `candidate` and `failed`, `deployments[]` with remaining trials, `rollback`, `install`, `last_action` and the credential-free part of `lifecycle`) |
...
| Journal excerpt | 400 lines, 128 KiB, 1 KiB per line, warning and worse, this boot | micad's `diagnostics.rs`, newest lines kept; `journalctl` killed at 5 s |
| Failed units | 64 listed, the count always reported; 3 s | micad's `diagnostics.rs` |
| Failed tasks | 32 | apid's collector |
| Manifest rows | 4096 | micad's `system_info.rs` |
| Telemetry lists | 32 each | micad's `telemetry.rs` |
| Wireless interfaces asked | 4, two bounded exchanges each (`STATUS`, `SIGNAL_POLL`; 1 s apiece) | micad's `network_state.rs` |
...
**Retention.** The store lives at `/mica/diagnostics`, in the system-owned
...
operation. `mica-data-layout` creates the store directory at boot with mode
...
| Route | Auth | Answers |
|---|---|---|
| `GET /api/v1/system/info` | bearer or session | 200 the section 2 surface; 401; 500/503/504 as every micad read; 405 |
...
| `GET /api/v1/diagnostics/snapshots/{id}` | bearer or session | 200 the document, `Content-Disposition: attachment; filename="mica-diagnostics-<machine id prefix>-<id>.json"`, `Cache-Control: no-store`; 404 `snapshot_not_found` |
...
store, and both write only under `/mica/diagnostics`. The three status reads
pass micad's answer through the live denylist like every other read; the
snapshot export serves stored bytes that were redacted at collection.

## 9. Bus surface

`com.mica.micad1` gains four read-only members beside the table in micad.md:
...
one place micad runs a program to answer a read (`journalctl`, fixed
...
- **Symptom:** writes or update staging fail because DATA is full.
  - **Evidence:** the `data` entry in `storage.tiers[]` has
    ...
    - **Remediation:** remove no-longer-needed diagnostic snapshots and
      ...
      unknown files from `/mica`; confirm pressure clears after space is freed.
  ...
  - **Evidence:** the DATA tier is read-only/unmounted, or the `/mica` bind in
