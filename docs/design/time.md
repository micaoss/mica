...
> Always-running network time on the systemd/micad base, a DATA/state-backed
> last-known-good clock floor, and a presentation-only timezone. Companion to
> micad.md, api.md and ro-root.md.

## 1. What ships

- **`systemd-timesyncd`**, installed by `mica-system` and enabled statically
  ...
  UI.
- **Pinned base policy** in `/etc/systemd/timesyncd.conf.d/50-mica.conf`:
...
- **A micad reconciler** (`reconciler/time.rs`) rendering both into runtime
...
Instead, micad owns persistence (the settings tree) and application:

- the reconciler renders the zone name to **`/run/mica/timezone`** (one line),
...
- validation is **syntactic and deterministic** at every write surface
  (`micad_settings::validate_timezone_name`, the tzdata name grammar — no
...
`time.ntp.servers` renders to `/run/systemd/timesyncd.conf.d/60-mica-servers.conf`
(`NTP=` under `[Time]`; `60-` so it sorts after the pinned `50-mica.conf` and
...
The server-list grammar (`micad_settings::validate_ntp_servers`: hostname/IP
charset, 253-byte bound, at most 8, no duplicates) is stated once in
micad-settings and enforced through `Settings::set` itself, so no write
...
`GET /api/v1/time/status` (read-only, authenticated) serves micad's
