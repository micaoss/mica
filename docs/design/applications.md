...
This document defines the planned local application product for mica. It is an
...
routes, and the image has no `mica-appd` implementation.
...
runtime units or `com.mica.*` application services; a discovered service need
...
`mica-appd` is the proposed lifecycle owner behind micad:

```text
Browser / kiosk -> APID -> micad -> mica-appd -> OCI/native adapter -> systemd
                         |          |              |
...
APID continues to expose only the versioned Mica OS management API. micad calls the
...
`com.mica.*` application-data namespace. The Item1/MQTT contract remains an
...
|---|---|---|
| Registry, active revision, small activation metadata | `/mnt/data/state/mica/apps/` | DATA/state; survives reboot and A/B |
| Download staging and artifact cache | `/mica/apps/.staging/` and manager-owned cache | DATA; bounded and garbage-collected |
| Per-app persistent data | `/mica/apps/<app-id>/data/` | DATA; retained by default on remove |
