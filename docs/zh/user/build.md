...
在 amd64 工作站上，arm64 那一半是在模拟下构建的。多数情况下它能复现已发布的字节——
基础软件包和容器引擎都实测如此——但并非总是：`mica-core` 的 Rust 包在模拟下会不同，
而它们的 amd64 构建逐字节相同。无论哪种情况，离线构建得到的都是一个**可用的**系统；
只有按架构原生构建的 CI，才能回答某个 arm64 产物是否仍与它的发布一致。
