...
的。如果构建容器运行在目标平台上，本地构建就是同一个构建、只是跑在模拟里，它能复现
——基础软件包和容器引擎都实测如此。如果它运行在宿主平台上用交叉工具链，而 CI 是在该
架构的 runner 上原生构建，那就是两个不同的构建：`mica-core` 的 arm64 包在本地是交叉
编译出来的，与原生构建并发布的那批不同。无论哪种情况，离线构建得到的都是一个**可用
的**系统；只有 CI 才能回答某个 arm64 产物是否仍与它的发布一致。
