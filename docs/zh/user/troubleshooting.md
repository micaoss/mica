...
Mica OS 上的诊断遵循一个顺序：获得访问，精确识别正在运行的是什么，读取系统
...
可用时的原因——一份 Mica OS 行彼此不一致的软件包清单会把这个不一致连同它找到的
...
是什么样"的答案。在 shell 里，`/usr/share/mica/manifest.tsv` 就是 API 读的
...
- **启动健康：**健康门记录它所需的集合、每个成员的裁决，以及它看到的每一个
  失败 unit（`journalctl -u mica-health`）。更新后持续回退的部署，失败在所需成员
  之一：启动事务稳定、micad 应答、apid 监听——日志会点名是哪一项。单个失败的
  unit **不会**让部署回滚；它被上报到 live-state 的 `health.units`，用
  `GET /api/v1/state/health` 或从一份诊断快照里读。健康门要求什么写在只读根里的
  `/etc/mica/health.conf`。
...
经验法则：Mica OS 的拒绝信息就是设计来逐字引用给支持或写进 issue 的；不要
