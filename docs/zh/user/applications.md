...
一个点名的小集合——启动事务已完成、micad 应答、apid 应答——应用的 unit 不在
...
`com.mica.<class>[.<suffix>]` 下暴露 `com.mica.Item1` 接口，`mica-mqttd` 桥
...
`mica-ui-pack` 生成的 `.mica-ui.zip`；系统校验后把它作为未激活版本保留在 DATA 的
`/mica/ui`。可以保留多个版本、精确选择一个在 `/` 提供、回到内置 UI，并删除 inactive
...
只要求这个部署还能被**恢复**——启动事务已完成、micad 应答、apid 应答——其余
...
> status: shipped — evidence: `mica-system:overlay/usr/lib/mica/mica-health`, `docs/design/native-applications.md`
