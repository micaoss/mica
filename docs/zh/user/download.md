...
**已发布的 `uefi` 镜像无法启动**（2026-09-19）。`20260916-0845`、
`20260916-1653` 和 `20260919-2103` 里的每一个 `uefi-x64` 与 `uefi-arm64` 镜像，都会在
PID 1 拒绝镜像自己签名身份里的板卡名，并在 1.7 秒时把机器关掉：板卡改了名，而读这个
名字的被 pin 住的客户端没有改。`cx3576` 不受影响——它的名字没变。不删除任何东西；
由一个修正后的发布取代它们，本页会在那个发布存在时写出它。在那之前，这些文件能下载，
但不能运行（[构建门](../../design/build-harness.md) 第 4 节）。
