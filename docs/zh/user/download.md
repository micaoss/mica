...
只有发布目标板的产品才有镜像和更新归档；自 `s905x5m.20260920-0033` 起，四块板
全部是发布目标——`uefi-x64`、`uefi-arm64`、`cx3576` 和 `s905x5m`，索引
`mica.20260920-0046` 里共八个产品。发布的产品都是 `dev` 或
...
**请取 `20260920-0622` 或更新的发布**——四块板在该时间戳上的发布，索引
`mica.20260920-0636`，出自 `73aca2c`。镜像里有两处修复，刷写前两处都重要：

- `20260919-2356`（索引 `mica.20260920-0008`，出自 `f46b64a6`）修的是板卡名：它
  pin 的 `mica-core` 发布 `20260919-2226`，在那个曾经缺失、导致更早的镜像在 PID 1
  断电的分支里匹配 `uefi-x64` 与 `uefi-arm64`。这一轮的两个 `uefi-x64` 产品在各自的
  发布运行里**被真的启动过**并到达通过标记；启动步骤只在 amd64 上执行，其余产品只被
  构建和静态校验，没有被启动。
- `20260920-0622` 修的是**控制台登录**，在它之前没有任何镜像有：`/etc/pam.d/login`
  与四个 `common-*` 缺失，导致任何板上都无法组装登录栈、任何账户都登不进去
  （[访问](../../design/access.md) 第 2 节）。经 micad 的 SSH 从未受影响。这些根
  现在有十六个 `/etc/pam.d` 文件，且所有 include 都能解析。

**已发布的镜像不等于被启动过的镜像**，而整个目录正好在这一点上对半分开：八个产品里
有四个——`cx3576-dev`、`cx3576-prod`、`s905x5m-dev`、`s905x5m-prod`——没有任何自动
流程会启动它们，因为没有任何套件会启动一个 FIT 镜像。它们每次推送都会被检查，只是
那些检查从不启动它们。（用户于 2026-09-20 报告 `cx3576` 在实机上启动成功；那是一条
报告，不是合格行——见[支持层级](../../boards/support-tiers.md)。）`uefi-x64` 镜像在每次推送和每次发布里
都会被启动，`uefi-arm64` 则只有早于板卡改名的手工 QEMU 记录
（[构建门](../../design/build-harness.md) 第 4 节）。成为
发布目标只意味着镜像被构建、发布并进入索引，它不是关于硬件的断言
