...
> status: shipped — evidence: `mica-build:boards/uefi-x64/board.env`, `mica-build:boards/uefi-arm64/board.env`, `mica-build:boards/cx3576/board.env`

## 文件放在哪里

| 路径 | 用途 |
|---|---|
| `/mica/config` | 通过 API 管理的设备配置 |
| `/mica/apps`、`/mica/containers` | 托管应用和容器数据 |
| `/srv` | 操作者文件与应用数据 |
| `/home`、`/root` | 由 `/mica` 支撑的持久用户目录 |
