...
**开放问题，也是第一个“板卡属性可能约束归档类型”的案例。** 复用由**输入决定，不由
字节决定**：`mica-boards` 把组件的 `mica.inputs` 与该板最新发布比较，因此**loader 输入
未变的发布会按同一摘要重新发布、不重新构建**，什么也不会不同。那条更窄、也是永久成立
的陈述是：**loader 输入变了的发布会重建它，而那次重建永远不会逐字节一致**，因为
`s905x5m` 的厂商签名是不确定性的。它的输入是该板的 `loader/`、`bsp.env`、板卡
`Makefile`、`common/uboot`、`common/scripts`、`common/trust`、两条厂商 git 行、工具链
source 行、`bsp` 镜像摘要与启动证书；真的发生时的代价是十二个文件里的四个、16.13 MiB；
频率跟随 loader 而不是日历——2026-09-16 那一周三次，不触碰它的那一周零次。

所以这个开放问题比初看时更小：「loader 变动本身是否迫使发 `full` 归档，还是归档类型只
按 root 与 kernel 标识计算、loader 随行」，决定的是**重建 loader 的那些发布**的代价，
而不是每一次发布的代价；`s905x5m` 并未在结构上被排除于部分更新之外。由 `mica-build`
依据它的代码和前两个发布来回答，而不是依据设计意图，也不在这里回答。它记在规则旁边而
不是那块板的页面里，因为答案关乎归档类型，而不关乎那块板。
