...
Only B3 authorized boot scripts/wiring, boot.rs and narrow mica-init contract,
...
The independent copy preflight is implemented in `pkgs/mica-deploy/src/boot.rs`.
All names, conflicts, permissions and sizes are checked before creating anything.
The destination must be the fresh empty tmpfs already supplied by mica-init.
...
Proposed extension, not implemented: a narrow native `mica-shutdown` PID-1
supervisor, with `src/bin/mica-shutdown.rs` and `src/boot/shutdown.rs`, sharing
its lifecycle contract with partial-startup mica-init refusal. It owns the
...
also needs narrow export wiring in `pkgs/mica-deploy/hack/build-deb.sh`
(`ALL_BINARIES`) and `build/src/kernel-package.ts` (boot input copy); these
paths were identified from the existing mica-init producer but not modified.
An approved reuse of the mica-init binary could avoid new export wiring at the
...
Evidence: `/tmp/mica-b3-checks.hMNe51/`; persistent-shell tmux
...
- `timeout 180 bash hack/check.sh`, unchanged, in the image resolved by
  `build-env/from.sh --arch=amd64 --ref LOCAL_MICA_BUILD_RUST_CHECK`: exit 0;
...
- `timeout 120 make os-shell-pipefail-lint`: exit 2, 154/155 clean. Sole accepted
  ...
  `FAIL: pkgs/micad/apid/ui/verify-ui-policy.sh:82: an early-exiting grep on the right of a pipe, in a file that sets pipefail: the pipeline reports failure when the pattern IS found. Use 'grep -c ... >/dev/null'`.
...
- pma-rust acceptance and pma-cr local security/storage review cover the two
  changed Rust files and the direct mica-init copy caller. PASS for this partial
...
campaign `mica-open-plans-20260910-100408`. This continues the same claim and
...
- `pkgs/mica-deploy/src/bin/mica-shutdown.rs`, `src/boot/shutdown.rs` and the module
  declaration in `src/boot.rs`; existing `src/bin/mica-init.rs` only for bounded
  command supervision, incremental acquisition, retained payload and refusal.
- `pkgs/mica-deploy/lifecycle-sys/Cargo.toml`, `lifecycle-sys/src/lib.rs` and optional
  `lifecycle-sys/tests/ioctl.rs`; necessary member/path dependency/rustix feature
  and lock edits in `pkgs/mica-deploy/Cargo.toml` and `Cargo.lock` only.
...
- `pkgs/mica-deploy/hack/build-deb.sh` only explicit new-binary build/export checks;
  `pkgs/mica-boot/initramfs.sh` and `Dockerfile` only authenticated same-architecture
...
- Focused `pkgs/mica-deploy/tests/shutdown.rs`, existing `tests/exitrd.rs`, new
...
The same safe module serves mica-init refusal; no duplicate cleanup policy.
...
2. RED deterministic live-graph transition/refusal and partial acquisition fixtures;
   implement bounded ordinary unmount, verified holder-first Mica OS DM removal,
...
- Native ownership also records Linux sysfs `diskseq` for backings, Mica OS mappings
...
then holder-first verified Mica OS DM removal, released loops and final backing
...
`/tmp/mica-b3-checks.hMNe51/`, including the original partial failures. The gate
...
| `native-package-export.commands/packaged-native.sh` | PASS: production build-deb explicit mica-init/mica-shutdown export on amd64/arm64; hashes match component builds |
...
|---|---:|---:|
| Native mica-init | 1,691,784 | 1,734,584 |
| Native mica-shutdown | 1,333,152 | 1,360,848 |
...
reduction. There is no previous actual B2 native mica-init artifact to support an
...
unmeasured B7 rows. Startup includes two copies of mica-shutdown (early worker
and retained entry), both included in the archive figures.

- x64 mica-shutdown SHA256: `426903654074de89d6fb821bc6a446bdc45bebbb4d57b965d173b10f2056b410`.
- aa64 mica-shutdown SHA256: `c4f1581a37143025ad4c4989738d2dcc779e02ba8e8244ba639f95b7fba3ccf7`.
...
explicitly request `--bins "mica-init mica-shutdown"` and pass each architecture's
...
handoff for campaign `mica-open-plans-20260910-100408`, authorizes implementation
...
1. Reuse `src/boot/shutdown.rs` and `src/bin/mica-shutdown.rs`: replace external
...
3. `src/boot.rs`, `pkgs/mica-boot/initramfs.sh` and necessary existing payload wiring
   ...
   Startup retains BusyBox, blkid, veritysetup, dmsetup and mica-init setup.
4. Modify the actual native export `hack/build-deb.sh` and required
   ...
   pass explicit mica-init/mica-shutdown paths and use build-deb with both bins;
...
Inspect exact `localhost/mica-build-rust:amd64` image identity, rustc/cargo, target
...
static proof. Do not apply flags to mica-init, mica-deploy or host build scripts.
...
- GNU target-only `+crt-static` succeeded with the existing Rust 1.98.0 producer
  ...
  dynamic headers. Host build scripts and `mica-init` keep their existing route.
- Evidence root: `/tmp/mica-b3-static.3ttniist`. `preflight-{x64,aa64}.json/log`
...
- Necessary direct-consumer adjustments also include
  `pkgs/mica-deploy/tests/boot.rs` and
...
| --- | --- |
| `mica-shutdown` | 2,047,144 bytes; SHA-256 `d2c5c9a6e2473c0125670031c79014c6ee946b834e2e26f32a65f38939e68b35` |
| `mica-init` | 1,673,848 bytes; SHA-256 `738391aa650a58fb3819f52831f6affd57ddd17e357c2a161faaf39d800ec642` |
...
`/tmp/mica-b3-static.3ttniist/measurements.json`. Actual guest working allocation,
...
- `native-x64-reviewed`: actual `build-deb.sh --producer b3-static-amd64 --bins
  "mica-init mica-shutdown" --arch amd64` succeeds with private outputs, pinned
...
- `shell-final`: 161/162 files clean; the only failure is the preserved unrelated
  `pkgs/micad/apid/ui/verify-ui-policy.sh:82` pipefail baseline. The new producer's
...
and verbatim `/tmp/mica-b3-static.3ttniist/serializer-claim.log` preparation failure.
...
Evidence: `/tmp/mica-b3-reopen.gc9d6pdo/reopen.json` and `reopen.log`, with before/
