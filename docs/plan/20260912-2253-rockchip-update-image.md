...
`make os-image-cx3576` writes `mica-cx3576-<ts>.img` (1299 MiB). The BSP
...
- `boards/cx3576/bsp/uboot/mica-file-boot.c` `valid_layout`: partition count,
...
- `rootfs/overlay/usr/lib/mica/mica-grow-data`: requires the SYSTEM PARTUUID,
...
- Runtime `micad` resolves partitions by partlabel.
...
3. **GPT identity.** The tool creates the GPT. Unless measured otherwise, the
   disk GUID and type GUIDs are not ours: `mica-grow-data` refuses the disk
...
- Run rkbin `tools/boot_merger` in `boards/cx3576/bsp/uboot/Dockerfile`;
  export `rk3576-mica-loader.bin` and its `idblock.img`.
...
- `mica-grow-data`: drop the PTUUID equality and the literal partition number;
...
- Wire it into the CX3576 image stage: publish `mica-cx3576-<ts>.update.img`
...
- Docs: rewrite the bsp README flashing section (RKDevTool upgrade path next
  to `flash-mica`), `docs/user/install.md` and `docs/zh/user/install.md`, the
...
- **Verification gap:** RKDevTool has no whole-disk readback. `flash-mica`
