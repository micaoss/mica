...
`pkgs/mica-boot/{kernel,fit,initramfs,compression,dev-keys,init-keys}.sh`,
...
MICAUPD1 archive in `build/src/component-archive.ts`; wrapping it in zstd does
...
updates. The device continues to receive authenticated raw MICAUPD1 archives.
