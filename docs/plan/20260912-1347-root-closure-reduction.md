...
| | |
|---|---|
| Composed root, unpacked | 200.4 MB, 2153 files, 11 selected mica packages |
...
| five Rust binaries (`apid` 12.2, `micad` 7.8, `mica-mqttd` 5.2, `mica-mqtt-broker` 4.9, `mica-deploy` 2.1) | 32.13 MB | 12.89 MB | 11.5 MB |
...
Debian arm64 archives plus the `pkgs/micad/` binaries from one cross build, and
...
1. **Section 3.1, health gate.** `mica-health` does not degrade to SKIP when
...
3. **Section 4, one workspace.** The five binaries span two workspaces.
   `mica-deploy` lives in `pkgs/mica-deploy/` with `lifecycle-sys`, and that
   workspace also produces the static `mica-init` and `mica-shutdown` that
   [20260911-1927](20260911-1927-boot-artifact-size.md) measures. The
   profile and multicall levers apply to the four `pkgs/micad/` binaries
   (30.1 MB unpacked); `mica-deploy` is out of this plan.
4. **Section 4, "touches one repository".** The four binaries ship as four
   packages from two producers (`micad` produces `micad` and `mica-apid`;
   `mqtt` produces `mica-mqttd` and `mica-mqtt-broker`). A multicall binary
...
**R1. Release profile for `pkgs/micad/`.** Add to the workspace `Cargo.toml`:
...
| Fat LTO with one codegen unit lengthens every micad build, including the arm64 cross build | Accept if R1's saving is material; otherwise record and drop to `lto = "thin"` |
...
- `pkgs/micad/Cargo.toml` (R1); a prototype crate under `pkgs/micad/` for R2
...
(section 6), `mica-deploy`, and everything listed under Alternatives.
...
|---|---:|---:|---|
| curl probe in Rust, drop curl and its closure | 3.90 MB | 3.76 MB | the apid health-check mode PLAN-086's policy table already designs; drop the wget fallback with it; `mica-system` Depends edit |
| host-key generation in Rust, drop `openssh-client` and the sftp server | 2.07 MB | 2.11 MB | `ssh-key` crate work in `mica-seed-state`'s replacement; `mica-system` Depends edit; SSH/SFTP acceptance |
| drop the iptables family | 0.21 MB | 0.20 MB | `mica-system` Depends edit; reverses the "operator habit" rationale its control file records |
| `setquota` via `quotactl(2)` | 0.54 MB | 0.66 MB | a native data-layout step; `mica-system` Depends edit |

**Section 7, micad programs the network.** Separate plan. Its acceptance
