...
1. **Brand split.** `README.md` now says Mica OS (`mica`, micaos.dev); every
   document under `docs/` (including website copy) still used the former project name. The split
   plan renames repositories to `mica-*` while package, binary and bus names
   kept the former project prefix; no document states that naming rule.
...
3. **Board status is stated in at least six places with different answers.**
   ...
   s905x5m "not a current Mica OS system-image release target". `board.env`:
...
1. **Broken language switches.** `boards/contract.md`, `design/micad.md` and
...
3. **Permanent documents cite deleted records by ID** (PLAN-070 ×7 in
   `micad.md`, PLAN-044/RFCT-280 in `time.md`, PLAN-088/089 and RFCT-359 in
...
1. Prose uses "Mica OS"; package, bus and path names keep their prefix
   until the split plan decides otherwise.
