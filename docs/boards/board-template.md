...
- `BOARD_CMDLINE_ARGS` console facts and any earlycon. Say which console is
  ...
  systemd write.
- Display console if any, and whether `BOARD_FEATURES` names `display`. A board that does
  takes the boot-logo and console-recoverability contract in
  `docs/design/display.md` §4.

### Peripherals

- Radios (`wifi`/`bluetooth` in `BOARD_FEATURES` and the concrete modules/SKUs).
