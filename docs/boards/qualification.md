...
> status: shipped — evidence: `docs/boards/board-template.md`
...
- BSP version (the synced-to upstream commit plus the Mica OS tree's own
  revision).
...
command line parameter `mica.profile` (`docs/design/access.md` §5.3). The evidence note names the profile, and a board
...
mica-qualified regardless of the rest.
...
> status: shipped — evidence: `mica-build:make os-fit-records-test`
