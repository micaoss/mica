...
- Product/board name: **uefi-arm64** — a generic system named for its firmware
  class, not for a machine: there is no vendor and no model designation. It was
  called `virt-arm64` until 2026-09-16
  ([decision](../decisions/2026-09-16-generic-systems-named-by-firmware.md)),
  after the QEMU machine that is still its only evidence.
...
- `MICA_ARCH`: `arm64`.
- Board directory: `boards/uefi-arm64/`.
- The board name is deliberately not `arm64`. `boards/uefi-x64/board.env`
  states that the architecture is declared rather than derived from the board
  name, and a board named only for its architecture would collapse
  `MICA_BOARD` and `MICA_ARCH` into one token and collide with the `out-arm64`
  artifact paths. `uefi-arm64` names the firmware class first and carries the
  architecture as its suffix.
...
- One revision, `all`. A virtual machine has no hardware revisions; what varies
  ...
  alias exactly as uefi-x64 takes the unversioned `q35`.
...
- Status indicator: none. `BOARD_FEATURES` names no `status-led`; `mica-status-led.service` is
  not installed.
...
the qualification table below. No digest from the retired
...
trial fallback. This is development-grade evidence
under disposable AAVMF enrollment, without hardware-rooted or production-key
claims. See `boards/uefi-arm64/evidence.json`.
