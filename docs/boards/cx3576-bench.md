...
[cx3576.md](cx3576.md) and the 39 finer detail rows the collector reports.
...
3. **A markdown report**, printed by `collect.sh report`, with the
...
| A host PC for the USB gadget console | 7 | `mica-gadget` binds a CDC ACM getty |
...
`mica-core:tests/apid-api`'s phases use: **a stage whose assumption is unmet is
...
- **1 comes with 5's growth half and 11, because first boot happens once.**
  ...
  document are minted on that boot; `mica-machine-id` writes `machine_id` into
...
Use an explicit `MICA_IMAGE`; never allow the make target's newest-file fallback
...
Collect `mica-deploy status`, `mica-deploy firmware-readback`, `/run/mica/boot.json`,
...
Collector v3 evaluates `boot-settled`, `micad` and `apid` from the shipped
required-health log, retains optional failed-unit names without treating them
as required failures, and probes live `micad.GetState("")` and the confirmed
...
(`[Match] Name=eth*`, `[Network] DHCP=yes`) and anything micad rendered into
...
what networkd said about the one that did not, and what MAC `mica-mac` gave it.
...
- **Scriptable.** `ip -details link show can0` (bitrate 250000, `fd off`,
  ...
  {idVendor,idProduct,product}`; the `mica-otg`, `mica-can`, `mica-gadget` and
  `mica-modules` unit states.
...
Row D5 (HDMI panic output) is not a pass criterion: the console policy routes
kernel and service output to serial only. The existing
...
Import a signed `MICAUPD1` archive, then install its verified descriptor and
objects with `mica-deploy`. Reboot and confirm the resulting authenticated ID.
...
- `/mica`, `/srv` and `/mica/containers` have zero byte and inode quota limits;
...
3. Return from tty2 and record the resulting presentation. The kernel's bounded redraw path is
   proven by host fixtures only; physical behavior remains unqualified until observed on
   the exact image.
4. Boot headless, bank connector/fb0 state, attach the named sink after the
   ...
   redraw path retains the artwork and handles the viewport callback, but
...
the current repository, so these rows remain blocked until the operator
supplies them.
...
the kernel log (`echo mica-pstore-<date> > /dev/kmsg`). After it, list
...
in the journal and reports it at live-state `health.units`. It is **not a
health-gate failure and not a rollback**: the gate confirms on a required set,
and this unit is not in it. The dash is absent deliberately, so that the
failure is visible rather than swallowed. If it fails here, the finding is still the image's, not
the board's. The probes: `systemctl status mica-regdb-reload.service`,
...
### The health gate's required set

Three probes that only a booted device can settle: they concern what a device
does when something is wrong, which no offline test can observe.

| Probe | Stage | Claim |
|---|---|---|
| `health-required-set` | 1 `firstboot` | every member of the shipped required set passes on a real boot |
| `health-tolerates-failed-unit` | 4 `network` | a failed unit outside the set does not cost a trial attempt |
| `health-rejects-broken-deployment` | 8 `update` | a deployment missing a required member still rolls back |

**`health-required-set` (stage 1).** Read `journalctl -u mica-health` and
`systemctl show -p Result --value mica-health.service`. **Pass:** the log carries
`required set: boot-settled micad apid`, one `required member <m>: OK` line for
...
masking `apid.service`, or `micad.service`, in the bundle's root is the smallest
one that is not also a broken kernel. **Pass:** the bad deployment boots, the gate
logs `required member apid` (or `micad`) with no confirmation, and the deployment rolls
...
.../UDC`, and `journalctl -u mica-gadget.service`. **Pass:** `acm.usb0` is a
...
### Stable MAC assignment

This board's Ethernet MAC addresses are derived from the port's path through
the bus topology, not its interface name, and two files make the assignment
reach both ports. Every
...
md5, `ID_NET_LINK_FILE` names `60-mica-mac-stable.link`, and udevd logged no
...
exactly as designed — on an address Mica OS did not choose.
...
for `/mica`, `/srv` and `/mica/containers`; no complete image bound to the current
...
row. Keep the exact evidence with the board dossier.
...
- **Output.** `--out DIR`, defaulting to `/srv/bench` — the user-owned
  namespace on DATA, which survives a reboot and a power cut. The collector may fall back to `/tmp/mica-bench`, which is volatile and
...
- **The API.** `GET /api/v1/update` and the other reads are authenticated.
  ...
  or `MICA_BENCH_TOKEN`; without either one the collector records which input is
...
- **Present:** `bash`, `sh`, `curl`, `networkctl`, `journalctl`, `systemctl`,
  `mica-deploy`, `podman`, `udevadm`, `dmesg`, `lsblk`, `date`, `stat`, `awk`, `sed`,
...
`collect.sh report` prints the thirteen rows in the dossier's own
column order followed by the 39 current details. Review every cell against the
raw evidence;
then update [cx3576.md](cx3576.md) in one change:
...
The collector's explicit-binding, health-window, dry-run/refusal and 39-row
report behaviour is covered by `cx3576/tests/bench/collector-test.sh`. A
complete signed cx3576 image builds and passes static verification (126
checks); build success is not physical qualification or a flash instruction.
No same-image physical cx3576 acceptance result has been captured: live
health, login, quotas and reset isolation, display, watchdog, recovery,
workloads and power cuts all require the exact-image bench session above.
Native crun execution and NPU ownership remain open.
...
connected boot, tty2, return from tty2 and late HDMI attachment. The kernel
carries retained logo data, VT-return redraw and Rockchip hotplug redraw, proven
by host fixtures and a compiled ARM64 integration check; no physical D3/D4 or
kernel lock-scheduling pass exists, so the HDMI/VT observations remain
unqualified until observed on the exact image. The open late-HDMI work is
[its task](../task/20260910-0117-cx3576-late-hdmi-logo.md).
