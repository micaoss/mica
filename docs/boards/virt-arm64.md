...
release producers; it is not a release target (`BOARD_RELEASE_TARGET=0`). Its evidence describes emulated
...
- The board name is deliberately NOT `arm64`. `boards/x64/board.env` states that
  ...
  board named for its architecture would collapse `MICA_BOARD` and `MICA_ARCH`
...
- Kernel: mainline stable `linux`, tag pinned in
  `mica-boards:boards/virt-arm64/kernel/versions.env` and verified by the sha256 of
...
- Configuration: arm64 `defconfig`, plus `mica-boards:common/kernel/mica-required.fragment`
  and `mica-boards:boards/virt-arm64/kernel/config/virt-arm64.fragment`, resolved by
...
Kernel source and configuration are pinned under `mica-boards:boards/virt-arm64/kernel/`.
...
claims. See `boards/virt-arm64/evidence.json`.
