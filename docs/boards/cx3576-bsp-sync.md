...
`docs/boards/contract.md` is a **cross-board BSP contract** — the artifact
...
More importantly, `tools/docs/verify-index.sh` asserts `docs/design/*.md` against
...
`5e2b1c3` has been reviewed, and everything applying to Mica OS has been ported. Four
...
| `a00c0ca` *"chore: ignore local host tools"* | **No-op for Mica OS** — edits upstream's own `/tools/` and `/yocto/` `.gitignore` entries; Mica OS vendors neither layout |
...
| `5e2b1c3` *"chore: remove unused Yocto BSP"* | **No-op for Mica OS** — deletes upstream's `yocto/` tree, which Mica OS has never vendored |
...
This is a **register, not a changelog**. It lists only places where Mica OS
...
**Mica OS A/B variant it can**: that build stores a persistent redundant environment
...
reason it matters to Mica OS is different from the reason it matters upstream, and
