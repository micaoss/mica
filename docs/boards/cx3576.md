...
- **SoC:** Rockchip RK3576.
- **MICA_ARCH:** `arm64`.
- **Board directory:** boards/cx3576/ (`mica-boards:boards/cx3576/board.env`).

## Provenance

Input trees and their relationships:

- **Board BSP tree** (`mica-boards:boards/cx3576/`): drifted derivative of
  <https://github.com/micaoss/board-cx3576-alpine> (a move the user has
  announced and not yet done), synced to upstream commit
  ...
  the sync record
  [docs/boards/cx3576-bsp-sync.md](../boards/cx3576-bsp-sync.md). Not a
...
> status: board-dependent — evidence: `mica-boards:boards/cx3576/loader/Dockerfile`
...
- **Lifecycle owner (BSP sync, CVE response, requalification):** Mica OS core —
  the board is the in-tree porting reference; no integrator of record.
- **Qualification owner:** Mica OS core; no matrix run is on file yet.
- **Escalation contact:** the Mica OS project (no per-board contact recorded).

## Boot chain

BootROM → vendor DDR/SPL/TF-A stages → pinned Mica OS U-Boot → required signed FIT
configuration → kernel and authenticated native init → SYSTEM ext4 → signed
root/support verity mappings → systemd. The Mica OS firmware embeds the public boot
...
> status: board-dependent — evidence: `mica-boards:boards/cx3576/loader/mica-file-boot.c`, `mica-build:tests/suites/lifecycle-uboot-fit/firmware-io.sh`, `mica-build:tests/suites/lifecycle-uboot-fit/signatures.sh`
...
geometry is `mica-boards:boards/cx3576/board.env`. FIRMWARE covers the loader
...
- **hwinit concerns** (`BOARD_HWINIT_CONFS="otg can bt mac gadget
  ...
  per concern under
  mica-boards:boards/cx3576/package/hwinit/ (`mica-boards:boards/cx3576/package/hwinit/hwinit-can`), reading
...
- **RTC:** the board device tree declares an `AT8563`/`hym8563` at `0x51` on
  ...
  the DATA/state-backed clock floor and the timezone setting ship and are the same
  on a board with no RTC at all.

> status: board-dependent — evidence: `mica-boards:boards/cx3576/kernel/dts/rk3576-cx3576z.dts`, `docs/design/time.md`
...
transport when the loader cannot run. The reflash is the operator's
`rkdeveloptool` procedure of [`docs/hardware/cx3576.md`](../hardware/cx3576.md):
write the whole image, read the loader region and then the rest back and
compare, reset only then. Physical execution remains pending.
...
Mica OS eMMC policy. See [installation](../user/install.md) and
[recovery](../user/recovery.md).

> status: board-dependent — evidence: `docs/hardware/cx3576.md`, `docs/boards/cx3576/rkdeveloptool/README.md`, `mica-core:micad/src/reset.rs`
...
> status: board-dependent — evidence: `mica-boards:boards/cx3576/evidence.json`, `mica-build:tests/suites/lifecycle-uboot-fit`
...
| Offline service | not tested | — | needs bench hardware (QEMU offline runs exercise uefi-x64, not this board) |
