...
echo "tools/docs/verify-coverage.sh: en/zh coverage rows in $TABLE <-> docs/{$(IFS=,; echo "${TREES[*]}")}/"
...
# --- verdict ---------------------------------------------------------------
if [ "$FAIL" -ne 0 ]; then
    echo "tools/docs/verify-coverage.sh: $FAIL FAILED, $CHECKS passed" >&2
    exit 1
fi
echo "tools/docs/verify-coverage.sh: $CHECKS/$CHECKS PASS"
